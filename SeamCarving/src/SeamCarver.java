...
public class SeamCarver {
	...
	//no performance gain from booleans if calling 50 remove rows after each other
	//can be solved by: adapting the remove method, multiple arraycopies per line (would be more calculation intensive)
	// or better by adapting the findSeam method so it works in the other direction
	...
	private int[] findVerticalSeam(boolean vertical) { 
		...
		// reversing this would speed up removeseam(findseam()) actions
		int width, height;
		int[][] currentEnergy;
		if (!vertical) { 
			width = rows;
	...
	private void recalcSeamBorders(int[] seam, boolean vertical) {
		if (vertical) {
			for (int i=0 ; i < seam.length ; i++) {
				int index = seam[i];
				if (index > 0) { //on the left and right borders only 1 value needs to be calculated
					energyTransposed[i][index-1] = (int) energy(index-1,i);
				}
				if (index < cols) //provided that cols is updated
					energyTransposed[i][index] = (int) energy(index,i);
			}
		}
		else {
			for (int i=0 ; i < seam.length ; i++) {
				...
				if (index < rows) // provided that rows is updated
